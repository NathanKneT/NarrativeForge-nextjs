...
test.describe('Asylum Story Flow', () => {
  ...
  test('should navigate through a basic story', async ({ page }) => {
    ...
    await expect(page.getByText("Bonjour c'est bien les secours")).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(
      page.getByText('Je viens tout juste de me réveiller')
    ).toBeVisible();
  ...
  test('should navigate to editor and back', async ({ page }) => {
    ...
    await expect(page.getByText('Story Editor')).toBeVisible({
      timeout: 10000,
    });
  ...
  test('should handle error states gracefully', async ({ page }) => {
    ...
    await expect(page.getByText('404'))
      .toBeVisible()
      .catch(() => {
